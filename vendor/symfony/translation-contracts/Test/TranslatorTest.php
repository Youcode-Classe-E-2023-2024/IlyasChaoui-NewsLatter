 ...
 * and Plural form mentioned on http://www.gnu.org/software/gettext/manual/gettext.html#Plural-forms.
 *
 * See also https://developer.mozilla.org/en/Localization_and_Plurals which mentions 15 rules having a maximum of 6 form.
...
class TranslatorTest extends TestCase
{
    ...
    public static function getChooseTests()
    {
        return [
            ...
            // Empty plural set (2 plural form) from a .PO file
            ['', '|', 1],
            // Empty plural set (3 plural form) from a .PO file
            ['', '||', 1],
    ...
    protected function validateMatrix(string $nplural, array $matrix, bool $expectSuccess = true)
    {
        foreach ($matrix as $langCode => $data) {
            ...
            if ($expectSuccess) {
                $this->assertCount($nplural, $indexes, "Langcode '$langCode' has '$nplural' plural form.");
            } else {
                $this->assertNotEquals((int) $nplural, \count($indexes), "Langcode '$langCode' has '$nplural' plural form.");
            }
        }
    }
